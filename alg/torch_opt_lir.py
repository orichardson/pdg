...
def _cpd_to_tensor_for_edge(mu, cpd_obj, X, Y):
	if isinstance(cpd_obj, ParamCPD):
		...
		cpt2 = mu.broadcast_torch(cpt, df=cpd_obj.cpd)
...
def torch_score(pdg, μ, γ):
    """
    Inconsistency objective for fixed μ (bits). Gradients flow ONLY to CPDs (θ).
    """
    loss = torch.tensor(0., dtype=torch.double)
    for X, Y, cpd, α, β in pdg.edges("XYPαβ"):
        muxy = μ.prob_matrix([X, Y])         # (|X|, |Y|)
        muy_x = μ.prob_matrix(Y | X)         # (|X|, |Y|)

        logcond_info = - tlog(twhere(muxy == 0, 1., muy_x))

        if cpd is None:
            loglikelihood = 0.
            logcond_claimed = 0.
        else:
            cpt, claims = _cpd_to_tensor_for_edge(μ, cpd, X, Y)
            loglikelihood   = twhere(claims, -tlog(twhere(cpt == 0, LOGZERO, cpt)), 0.)
            logcond_claimed = twhere(claims, logcond_info, 0.)


        loss += β * zmul(muxy, (loglikelihood - logcond_claimed)).sum()
