...
def _marginalize_slow(mu, shape, IDXs):
	"""Slow reference implementation of marginalization.

	Iterates element-wise over the flattened joint and accumulates into the
	appropriate marginal bin using unravel/ravel index conversions.
	"""
	if len(IDXs) == 0:
		return cp.sum(mu, keepdims=True)

	postshape = tuple(shape[i] for i in IDXs)
	# preallocate list of the appropriate size. Will hold scalar cp expressions.
	elts = [0] * np.prod(postshape, dtype=int)

	for i in range(mu.size):
		mui = mu[i]
		v_idx = np.unravel_index(i, shape)  # virtual index in joint shape
		idx = np.ravel_multi_index(tuple(v_idx[j] for j in IDXs), postshape)
